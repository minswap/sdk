...
import { Address, C,Constr, Credential, Data, getAddressDetails } from "lucid-cardano";
...
export namespace LucidCredential {
    ...
    export function toCSLStakeCredential(credential: Credential): C.StakeCredential {
  ...
  export namespace AddressPlutusData {
    ...
    export function fromPlutusData(networkId: NetworkId, data: Constr<Data>): Address {
      switch (data.index) {
        case 0: {
          const paymentCredential = LucidCredential.fromPlutusData(data.fields[0] as Constr<Data>)
          ...
          switch (maybeStakeCredentialConstr.index) {
            case 0: {
              ...
              const stakeCredentialConstr = maybeStakeCredentialConstr.fields[0] as Constr<Data>
              switch (stakeCredentialConstr.index) {
                case 0: {
                  const stakeCredential = LucidCredential.fromPlutusData(stakeCredentialConstr.fields[0] as Constr<Data>)
